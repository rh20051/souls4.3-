...
func physics_update(delta:float):
	...
	if enemy.global_position.distance_to(enemy.player.global_position) < 6  and angle_difference(enemy.rotation.y, enemy.angle +deg_to_rad(180)) < .1 and angle_difference(enemy.rotation.y, enemy.angle +deg_to_rad(180)) >= -.1:
